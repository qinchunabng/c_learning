...
int thr_channel_destroy(struct mlib_listentry_st *ptr){
    int i;
    for(i=0;i<CHNNR;i++){
        if(thr_channel[i] != NULL && thr_channel[i].chnid == ptr->chnid){
            if(pthread_cancel(thr_channel[i].tid) < 0){
                syslog(LOG_ERR, "pthread_cancel():the thread of channel %d.", ptr->chind);
                return -ESRCH;
            }
...
int thr_channel_destroyall(void){
    int i;
    for(i=0;i<CHNNR;i++){
        if(thr_channel[i] != NULL && thr_channel[i].tid > 0){
            if(pthread_cancel(thr_channel[i].tid) < 0){
                syslog(LOG_ERR, "pthread_cancel():the thread of channel %d.", ptr->chind);
                return -ESRCH;
            }
